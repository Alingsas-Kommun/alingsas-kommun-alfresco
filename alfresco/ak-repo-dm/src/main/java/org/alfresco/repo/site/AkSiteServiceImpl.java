 ...
 */
public class AkSiteServiceImpl extends SiteServiceImpl implements SiteServiceInternal, SiteModel, NodeArchiveServicePolicies.BeforePurgeNodePolicy, NodeServicePolicies.OnRestoreNodePolicy {
  /** Logger */
   ...
   */
  public void init() {
    logger.info("Initialized custom AK site service");
  ...
  protected void onBootstrap(ApplicationEvent event) {
    this.policyComponent.bindClassBehaviour(BeforePurgeNodePolicy.QNAME, SiteModel.TYPE_SITE, new JavaBehaviour(this, "beforePurgeNode"));
    this.policyComponent.bindClassBehaviour(OnRestoreNodePolicy.QNAME, SiteModel.TYPE_SITE, new JavaBehaviour(this, "onRestoreNode"));
  }
  ...
  public boolean hasCreateSitePermissions() {
    // NOTE: see ALF-13580 - since 3.4.6 PermissionService.CONTRIBUTOR is no
    // longer used as the default on the Sites folder
    // instead the ability to call createSite() and the Spring configured ACL is
    // the mechanism used to protect access.
    return (publicServiceAccessService.hasAccess("SiteService", "createSite", "", "", "", "", true) == AccessStatus.ALLOWED);
  }

  /**
   * @see org.alfresco.service.cmr.site.SiteService#createSite(java.lang.String,
   ...
   *      java.lang.String, java.lang.String, java.lang.String, boolean)
   */
  public SiteInfo createSite(final String sitePreset, String passedShortName, final String title, final String description, final SiteVisibility visibility) {
   ...
   * <p>
   * Creates the top-level site group, plus all the Role groups required for
   * users of the site.
   * <p>
   * Note - Changes here likely need to be replicated to the
   * {@link #updateSite(SiteInfo)} method too, as that also has to deal with
   * Site Permissions.
   * 
   * @param siteNodeRef
   * @param shortName
  ...
  private void setupSitePermissions(final NodeRef siteNodeRef, final String shortName, final SiteVisibility visibility, final Map<String, Set<String>> memberships, final boolean ignoreExistingGroups) {
    ...
    AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<Object>() {
      public String doWork() throws Exception {
        ...
        // From Alfresco 3.4 the 'site public' group is configurable. Out of the
        // box it is
        ...
        // This then is the group that is given SiteConsumer access to newly
        // created
        ...
        if (!PermissionService.ALL_AUTHORITIES.equals(sitePublicGroup) && !publicGroupExists && !SiteVisibility.PRIVATE.equals(visibility)) {
        ...
        /*
         * MNT-11289 fix - group is probably already exists. we should check it
         * for existence
         */
        ...
        if (!ignoreExistingGroups || !authorityService.authorityExists(siteGroup)) {
          siteGroup = authorityService.createAuthority(AuthorityType.GROUP, siteGroupShortName, siteGroupShortName, shareZones);
        }
        ...
        for (String permission : permissions) {
          ...
          /*
           * MNT-11289 fix - group is probably already exists. we should check
           * it for existence
           */
          ...
          String permissionGroup = authorityService.createAuthority(AuthorityType.GROUP, permissionGroupShortName, permissionGroupShortName, shareZones);
        ...
        if (SiteVisibility.PUBLIC.equals(visibility) == true && permissions.contains(SITE_CONSUMER)) {
          // The public site group becomes the consumer
          permissionService.setPermission(siteNodeRef, sitePublicGroup, SITE_CONSUMER, true);
        } else if (SiteVisibility.MODERATED.equals(visibility) == true && permissions.contains(SITE_CONSUMER)) {
   ...
   * 
   * @return Map<QName, Serializable> map containing the custom properties of
   *         the site
   */
        ...
        public NodeRef doWork() throws Exception {
          return retryingTransactionHelper.doInTransaction(new RetryingTransactionCallback<NodeRef>() {
            public NodeRef execute() throws Exception {
              ...
              List<NodeRef> results = searchService.selectNodes(rootNodeRef, sitesXPath, null, namespaceService, false, SearchService.LANGUAGE_XPATH);
              if (results.size() != 0) {
   ...
   * (non-Javadoc)
   * 
   * @see org.alfresco.service.cmr.site.SiteService#findSites(java.lang.String,
  ...
  public List<SiteInfo> findSites(String filter, String sitePresetFilter, int size) {
    ...
    if (siteRoot == null) {
      ...
      // The filter string is only used in the Lucene query if it restricts
      // results.
      // A search for name/title/description = "*" does not need to be put into
      // the Lucene query.
  ...
  /**
   * This method uses {@link CannedQuery canned queries} to retrieve
   * {@link SiteModel#TYPE_SITE st:site} NodeRefs with support for
   * {@link PagingRequest result paging}.
   */
  @Override
  public PagingResults<SiteInfo> listSites(List<FilterProp> filterProps, List<Pair<QName, Boolean>> sortProps, PagingRequest pagingRequest) {
    ...
    GetChildrenCannedQuery cq = (GetChildrenCannedQuery) getChildrenCannedQueryFactory.getCannedQuery(getSiteRoot(), null, null, searchTypeQNames, filterProps, sortProps, pagingRequest);

    // execute canned query
    final CannedQueryResults<NodeRef> results = cq.execute();

    // Now convert the CannedQueryResults<NodeRef> into a more useful
  ...
  /**
   * This method returns the {@link SiteInfo siteInfos} for sites to which the
   * specified user has access. Note that if the user has access to more than
   * 1000 sites, the list will be truncated to 1000 entries.
   * 
   * @param userName
   *          the username
  ...
  private String resolveSite(String group) {
    // purge non Site related Groups and strip the group name down to the site
    // "shortName" it relates too
  ...
  /**
   * Helper method to get the visibility of the site. If no value is present in
   * the repository then it is calculated from the set permissions. This will
   * maintain backwards compatibility with earlier versions of the service
   * implementation.
   * 
   * @param siteNodeRef
   *          site node reference
  ...
  /**
   * This method gets the <code>st:site</code> NodeRef for the Share Site which
   * contains the given NodeRef. If the given NodeRef is not contained within a
   * Share Site, then <code>null</code> is returned.
   * 
   * @param nodeRef
  ...
  private NodeRef getSiteNodeRef(final String shortName, boolean enforcePermissions) {
    ...
    if (siteNodeRef != null) {
      ...
      siteNodeRef = AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<NodeRef>() {
        public NodeRef doWork() throws Exception {
          ...
          // cache the result if found - null results will be required to ensure
          // new sites are found later
          if (siteNode != null) {
    ...
    if (enforcePermissions) {
      return siteNodeRef == null || !this.permissionService.hasPermission(siteNodeRef, PermissionService.READ_PROPERTIES).equals(AccessStatus.ALLOWED) ? null : siteNodeRef;
    } else {
      return siteNodeRef;
    }
  }
  ...
  public void updateSite(SiteInfo siteInfo) {
    ...
    if (currentVisibility.equals(updatedVisibility) == false) {
      ...
      if (SiteVisibility.PUBLIC.equals(currentVisibility) == true || SiteVisibility.MODERATED.equals(currentVisibility) == true) {
        ...
        // (Always remove both EVERYONE and the Publci Site Group, just to be
        // safe)
      ...
      // If the site was moderated before, undo the work of
      // #setModeratedPermissions
      ...
      // Note - these need to be kept in sync manually with those in
      ...
      if (SiteVisibility.PUBLIC.equals(updatedVisibility) == true) {
        ...
        // Set the moderated permissions on all the containers the site already
        // has
        List<FileInfo> folders = fileFolderService.listFolders(siteNodeRef);
        for (FileInfo folder : folders) {
          NodeRef containerNodeRef = folder.getNodeRef();
        ...
        // No additional permissions need to be granted for a site become
        // private
      }
  ...
  public void deleteSite(final String shortName) {
    // In deleting the site node, we have to jump through a few hoops to manage
    // the site groups.
    ...
    // no need to retain the membership of the site as we postpone delete of
    // authorities until purge from the trashcan

    // The default behaviour is that sites cannot be deleted. But we disable
    // that behaviour here
    // in order to allow site deletion only via this service. Share calls this
    ...
    // Postpone delete of associated groups to the time when NodeArchiveService
    // purges site node
    // because in case of recover ACLs and ACEs are needed which were set for
    // documents
  ...
  public void onRestoreNode(ChildAssociationRef childAssocRef) {
    // regenerate the groups for the site when it is restored from the Archive
    // store
    NodeRef siteRef = childAssocRef.getChildRef();
    setupSitePermissions(siteRef, (String) directNodeService.getProperty(siteRef, ContentModel.PROP_NAME), getSiteVisibility(siteRef),
  ...
  /**
   * @see org.alfresco.service.cmr.site.SiteService#listMembers(String, String,
   *      String, int, boolean)
   */
  ...
  public List<SiteMemberInfo> listMembersInfo(String shortName, final String nameFilter, final String roleFilter, final int size, final boolean collapseGroups) {
    ...
    if (tenantService.isEnabled() && (AuthenticationUtil.SYSTEM_USER_NAME.equals(AuthenticationUtil.getRunAsUser())) && tenantService.isTenantName(shortName)) {
      ...
      return AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<List<SiteMemberInfo>>() {
        public List<SiteMemberInfo> doWork() throws Exception {
          return listMembersInfoImpl(sName, nameFilter, roleFilter, size, collapseGroups);
        }
      }, tenantService.getDomainUser(AuthenticationUtil.getSystemUserName(), tenantDomain));
    } else {
  ...
  private Map<String, String> listMembersImpl(String shortName, String nameFilter, String roleFilter, int size, boolean collapseGroups) {
    ...
    List<SiteMemberInfo> list = listMembersInfoImpl(shortName, nameFilter, roleFilter, size, collapseGroups);
   ...
   * 
   * One of the user's firstname or lastname must match at least one of the
   * filters given.
   * 
   * @param filter
   * @param username
  ...
  private boolean matchPerson(final String[] nameFilters, final String username) {
    ...
    try {
      ...
      for (int i = 0; i < nameFilters.length; i++) {
        if (matchByFilter(lowUserName, nameFilters[i]) || matchByFilter(lowFirstName, nameFilters[i]) || matchByFilter(lowLastName, nameFilters[i])) {
          addUser = true;
          break;
        }
      }
  ...
  public SiteMemberInfo getMembersRoleInfo(String shortName, String authorityName) {
    ...
    QName siteType = directNodeService.getType(siteNodeRef);
    Set<String> permissions = this.permissionService.getSettablePermissions(siteType);
    // This set is a lazily evaluated one, so merely getting it in advance as we
    // do here is not expensive
   ...
   * Returns empty List if the user does not have a explicit membership to the
   * site.
   * 
   * A user permission will take precedence over a permission obtained via a
   * group.
   * 
   * @param siteShortName
   *          site short name
   * @param authorityName
   *          authority name
   * @return List<String> Permission groups, empty list if no explicit
   *         membership set
   */
  private List<String> getPermissionGroups(String siteShortName, String authorityName) {
    ...
    // First use the authority's cached recursive group memberships to answer
  ...
  /**
   * @see org.alfresco.service.cmr.site.SiteService#isMember(java.lang.String,
   *      java.lang.String)
   */
  ...
  public void removeMembership(final String shortName, final String authorityName) {
    ...
    if (role != null) {
      ...
      if ((currentUserName.equals(authorityName) == true) || (permissionService.hasPermission(siteNodeRef, PermissionService.CHANGE_PERMISSIONS) == AccessStatus.ALLOWED)) {
        // Run as system user
        AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<Object>() {
        ...
        if (authorityType == AuthorityType.USER) {
          activityService.postActivity(ActivityType.SITE_USER_REMOVED, shortName, ACTIVITY_TOOL, getActivityUserData(authorityName, ""));
        } else if (authorityType == AuthorityType.GROUP) {
          activityService.postActivity(ActivityType.SITE_GROUP_REMOVED, shortName, ACTIVITY_TOOL, getActivityGroupData(authorityName, ""));
        }
      } else {
        // Throw an exception
  ...
  public void setMembership(final String shortName, final String authorityName, final String role) {
    ...
    if (currentRole == null || role.equals(currentRole) == false) {
      ...
      if ((permissionService.hasPermission(siteNodeRef, PermissionService.CHANGE_PERMISSIONS) == AccessStatus.ALLOWED)
          || (SiteVisibility.PUBLIC.equals(visibility) == true && role.equals(SiteModel.SITE_CONSUMER) == true && authorityName.equals(currentUserName) == true && currentRole == null)) {
        // Check that we are not about to remove the last site manager
        ...
        if (currentRole == null) {
          AuthorityType authorityType = AuthorityType.getAuthorityType(authorityName);
          if (authorityType == AuthorityType.USER) {
            activityService.postActivity(ActivityType.SITE_USER_JOINED, shortName, ACTIVITY_TOOL, getActivityUserData(authorityName, role), authorityName);
          } else if (authorityType == AuthorityType.GROUP) {
            activityService.postActivity(ActivityType.SITE_GROUP_ADDED, shortName, ACTIVITY_TOOL, getActivityGroupData(authorityName, role));
          }
        } else {
          AuthorityType authorityType = AuthorityType.getAuthorityType(authorityName);
          if (authorityType == AuthorityType.USER) {
            activityService.postActivity(ActivityType.SITE_USER_ROLE_UPDATE, shortName, ACTIVITY_TOOL, getActivityUserData(authorityName, role));
          } else if (authorityType == AuthorityType.GROUP) {
            activityService.postActivity(ActivityType.SITE_GROUP_ROLE_UPDATE, shortName, ACTIVITY_TOOL, getActivityGroupData(authorityName, role));
          }
        }
      } else {
  ...
  public NodeRef createContainer(String shortName, String componentId, QName containerType, Map<QName, Serializable> containerProperties) {
    ...
    if (containerNodeRef == null) {
      ...
      if (containerProperties != null) {
        Map<QName, Serializable> props = this.directNodeService.getProperties(containerNodeRef);
  ...
  /**
   * This method recursively cleans the site permissions on the specified
   * NodeRef and all its primary descendants. This consists of
   * <ul>
   * <li>the removal of all site permissions pertaining to a site other than the
   * containingSite</li>
   * </ul>
   * If the containingSite is <code>null</code> then the targetNode's current
   * containing site is used.
   * 
   * @param targetNode
   * @param containingSite
   *          the site which the site is a member of. If <code>null</code>, it
   *          will be calculated.
   */
  @Override
  ...
  /**
   * Moderated sites have separate ACLs on each component and don't inherit from
   * the site which has consumer role for everyone.
   */
  private void setModeratedPermissions(String shortName, NodeRef containerNodeRef) {
    ...
    permissionService.setPermission(containerNodeRef, PermissionService.ALL_AUTHORITIES, PermissionService.READ_PERMISSIONS, true);
  ...
  /**
   * Helper method to create a container if missing, and mark it as a tag scope
   * if it isn't already one
   */
  public static NodeRef getSiteContainer(final String siteShortName, final String componentName, final boolean create, final SiteService siteService, final TransactionService transactionService,
  ...
  private void checkLastManagerRemoval(final String shortName, final String authorityName, final String role) {
    ...
    if (SiteModel.SITE_MANAGER.equals(role) == true) {
      ...
      Set<String> siteUserMangers = this.authorityService.getContainedAuthorities(AuthorityType.USER, mgrGroup, true);
