...
public class RevertDocumentExecutionStartListener implements ExecutionListener {
	private static final Logger LOG = Logger
			.getLogger(RevertDocumentExecutionStartListener.class);
	...
	public void notify(DelegateExecution execution) {
		...
		String akwfTargetChoice = (String) execution
		...
		if (CommonWorkflowModel.TARGET_CHOICE_OPTION_SITE
			...
			if (AccessStatus.ALLOWED.equals(permissionService.hasPermission(
				...
				execution.setVariable(CommonWorkflowModel.HANDLING,
						RevertDocumentWorkflowModel.OWNFOLDER_HANDLING);
				LOG.debug("User "
						+ akwfInitiator
						+ " have write permission on target folder, setting WF handling to "
						+ RevertDocumentWorkflowModel.OWNFOLDER_HANDLING);
				execution.setVariable(CommonWorkflowModel.APPROVER,
						akwfInitiator);
			} else {
				execution.setVariable(CommonWorkflowModel.HANDLING,
						RevertDocumentWorkflowModel.SITE_HANDLING);
				LOG.debug("User "
						+ akwfInitiator
						+ " have read permission on target folder, setting WF handling to "
						+ RevertDocumentWorkflowModel.SITE_HANDLING);
			}
			...
			if (containedAuthorities.size() == 0) {
				siteRoleGroup = siteService.getSiteRoleGroup(
						site.getShortName(), CommonWorkflowModel.SITE_MANAGER);
				containedAuthorities = authorityService
			...
			execution
					.setVariable(CommonWorkflowModel.SITE, site.getShortName());
			execution
					.setVariable(CommonWorkflowModel.SITE_GROUP, siteRoleGroup);

		} else if (CommonWorkflowModel.TARGET_CHOICE_OPTION_MY_HOME_FOLDER
				.equals(akwfTargetChoice)) {
			...
			NodeRef userHomeNode = (NodeRef) nodeService.getProperty(
					personNode, ContentModel.PROP_HOMEFOLDER);
			targetFolderNodeRef = userHomeNode;
			akwfTargetFolder = new ActivitiScriptNode(targetFolderNodeRef,
					serviceRegistry);
			execution.setVariable(CommonWorkflowModel.TARGET_FOLDER,
					akwfTargetFolder);
			execution.setVariable(CommonWorkflowModel.HANDLING,
					RevertDocumentWorkflowModel.OWNFOLDER_HANDLING);
			execution.setVariable(CommonWorkflowModel.APPROVER, akwfInitiator);
		} else {
			throw new org.apache.commons.lang.IncompleteArgumentException(
					"akwf:reviewDocumentTargetChoice=" + akwfTargetChoice);
		}
		...
		if (childAssocs != null && childAssocs.size() > 0) {
			for (ChildAssociationRef childAssoc : childAssocs) {
				...
				if (nodeService.getProperty(fileNodeRef,
						AkDmModel.PROP_AKDM_DOC_STATUS).equals(
						CommonWorkflowModel.DOC_STATUS_DONE)) {
					LOG.debug("Document is in status "
							+ CommonWorkflowModel.DOC_STATUS_DONE + ".");
				} else {
					throw new RuntimeException(
