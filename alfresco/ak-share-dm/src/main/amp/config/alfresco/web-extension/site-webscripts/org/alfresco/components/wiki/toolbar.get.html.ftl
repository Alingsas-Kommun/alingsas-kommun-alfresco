      ...
      <div id="${args.htmlid}-body" class="share-toolbar wiki-toolbar flat-button theme-bg-2">
         <div class="navigation-bar theme-bg-1">
            <div>
               ...
               <#if page.url.args.title! != "Startsida">
               <span class="forwardLink">
                  <a href="${url.context}/page/site/${page.url.templateArgs.site!""}/wiki-page?filter=main&amp;title=Startsida<#if args.showBackLink != "true">&amp;listViewLinkBack=true</#if>">${msg("link.mainPage")}</a>
               </span>
               </#if>
            </div>
         </div>
